...
type Stage = (typeof profile.journey.stages)[number];

function StageCard({ stage, index }: { stage: Stage; index: number }) {
  const [open, setOpen] = useState(false);
  const s = stage as Stage & { image?: string; honor?: string };
  const panelId = `stage-narrative-${index}`;

  return (
    <div className="flex h-full w-[82vw] max-w-sm flex-col rounded-2xl border border-paper/10 bg-paper/[0.03] p-8 transition-colors duration-500 hover:bg-paper/[0.06] sm:w-[400px] md:p-10">
      ...
      {s.image && (
        <div className="mt-6 w-fit border border-paper/15 bg-obsidian-soft p-1.5">
          <div className="relative aspect-[3/4] w-24 overflow-hidden bg-obsidian-soft">
            <Image src={s.image} alt={`${s.role} at ${s.org}`} fill sizes="96px" className="object-contain" />
          </div>
        </div>
      )}

      <h3 className="mt-6 font-display text-2xl leading-tight md:text-3xl">{s.role}</h3>
